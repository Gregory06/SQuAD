...
from modules import RNNEncoder, SimpleSoftmaxLayer, BasicAttn, BidirectionalAttn, BidirectionalAttnNew
...
class QAModel(object):
    ...
    def add_placeholders(self):
        ...
        self.con_qn_mask = tf.concat([self.context_mask, self.qn_mask], 1)
    ...
    def add_embedding_layer(self, emb_matrix):
        ...
        with vs.variable_scope("embeddings"):
            ...
            self.con_qn_embs = tf.concat([self.context_embs, self.qn_embs], 1)
    ...
    def build_graph(self):
        ...
        with vs.variable_scope("a1"):
            attn_layer = BidirectionalAttnNew(self.keep_prob, self.FLAGS.hidden_size * 2, self.FLAGS.hidden_size * 2)
        ...
        with vs.variable_scope("e2_1c"):
            encoder = RNNEncoder(self.FLAGS.hidden_size, self.keep_prob)
            context_hiddens_f = encoder.build_graph(attn_output, self.context_mask) # (batch_size, context_len, hidden_size*2)
        ...
        with vs.variable_scope("e3c"):
            encoder = RNNEncoder(self.FLAGS.hidden_size, self.keep_prob)
            context_hiddens_f_end = encoder.build_graph(context_hiddens_f, self.context_mask)

        blended_reps_end = tf.concat([context_hiddens_f_end, attn_output], axis=2)
        ...
        blended_reps_final_st = tf.contrib.layers.fully_connected(blended_reps_st, num_outputs=self.FLAGS.hidden_size) # blended_reps_final is shape (batch_size, context_len, hidden_size)
        blended_reps_final_end = tf.contrib.layers.fully_connected(blended_reps_end, num_outputs=self.FLAGS.hidden_size)
        ...
        with vs.variable_scope("StartDist"):
            ...
            self.logits_start, self.probdist_start = softmax_layer_start.build_graph(blended_reps_final_st, self.context_mask)
